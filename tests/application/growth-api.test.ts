...
test("crecimiento exige baseline congelado y conserva aislamiento sintético", async () => {
  const source = await readFile(routeUrl, "utf8");
  assert.match(source, /APPROVED_FROZEN/);
  assert.match(source, /SYNTHETIC_NON_COMMERCIAL/);
  assert.match(source, /sólo está habilitada para el Plan sintético aislado/);
});
...
test("building blocks separan familias y reconciliación neta", async () => {
  ...
  assert.match(source, /duplicateEconomicIdentities: 0/);
  assert.match(source, /unresolvedOverlaps: 0/);
});
