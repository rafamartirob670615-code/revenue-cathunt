...
async function requireAdministrator(request: Request, planId: string) {
  ...
  if (actor.email === ownerEmail) return actor;
  const row = await database().prepare(
    `SELECT 1 AS allowed
     ...
     WHERE lower(u.email)=lower(?) AND aa.capability='ADMINISTER_ACCESS'
       AND ((aa.scope_type='PLAN' AND aa.scope_id=?) OR aa.scope_type='ORGANIZATION')
...
export async function POST(request: Request) {
  try {
    ...
    if (!planId || !body.email?.trim() || !ASSIGNABLE_CAPABILITIES.includes(body.capability as AssignableCapability)) {
      throw new Error("Plan, usuario y capacidad son obligatorios");
    }
    const admin = await requireAdministrator(request, planId);
    const { plan } = await authorizePlan(request, planId);
    ...
    await database().prepare(
      ...
      `assignment:${membershipId}:${capability}:PLAN:${planId}`, membershipId, capability,
      "PLAN", planId, capability === "VIEW_FINANCIALS" ? '["FINANCIALS"]' : "[]", now, null, admin.email,
    ).run();
