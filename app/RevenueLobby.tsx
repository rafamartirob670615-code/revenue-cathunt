...
export default function RevenueLobby({ openPlan, openMonitor, createPlan }:{ openPlan:(planId:string)=>void; openMonitor:(planId:string)=>void; createPlan:()=>void; }) {
  ...
  return <div className="revenue-lobby">
    <section className="lobby-intro"><p className="eyebrow">Planeación comercial</p><h1>¿Qué quieres hacer?</h1><p>Construye un Plan anual por cuenta o monitorea el desempeño de un Plan oficial.</p></section>
    ...
    <section className="lobby-actions" aria-label="Opciones para comenzar">
      <article className="lobby-card real-card"><span className="lobby-step">01</span><div><small>Construcción</small><h2>Crear un Plan</h2><p>Define la cuenta y carga los datasets necesarios para construir, ajustar, guardar y aprobar el Plan anual.</p></div><button className="lobby-primary" onClick={createPlan}>Crear un Plan <strong>→</strong></button></article>
      <article className="lobby-card monitor-card"><span className="lobby-step">02</span><div><small>Seguimiento</small><h2>Monitorear un Plan</h2><p>Compara Plan, cuota, año anterior y actual por mes, trimestre, YTD y categoría.</p></div><button className="lobby-primary monitor" disabled={!monitorablePlans.length} onClick={()=>monitorablePlans[0]&&openMonitor(monitorablePlans[0].id)}>{monitorablePlans.length?"Abrir monitoreo":"Requiere un Plan aprobado"} <strong>→</strong></button></article>
    </section>
