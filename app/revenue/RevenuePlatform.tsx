...
export default function RevenuePlatform({ identity, initialModule = "monitoreo" }: { identity: RevenueIdentity; initialModule?: RevenueModule }) {
  ...
  async function approveCommercially(plan: Plan) {
    const version = plan.versions.at(-1);
    if (!version) return;
    setBusy("Aprobando comercialmente…");
    try {
      const response = await fetch("/api/plans", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "decide", planId: plan.id, versionId: version.id, approval: { id: `approval:${crypto.randomUUID()}`, stage: "COMMERCIAL", decision: "APPROVED", actorId: "authenticated-user", decidedAt: new Date().toISOString() }, context: { commandId: `decide:${crypto.randomUUID()}`, actorId: "authenticated-user", occurredAt: new Date().toISOString() } }) });
      const body = await response.json() as { ok: boolean; error?: string };
      if (!response.ok || !body.ok) throw new Error(body.error);
      setNotice(`${plan.accountName ?? plan.accountId} · ${plan.year} aprobado comercialmente.`);
      await loadHome();
    } catch (cause) { setError(friendly(cause instanceof Error ? cause.message : "")); }
    finally { setBusy(""); }
  }
...
function AdministrationModule({ plan, accounts, onChanged, existingPlans, onOpen, canApprove, onApprove }: { plan: Plan | null; accounts: AlfaUniverseAccount[]; onChanged: () => Promise<void>; existingPlans: Plan[]; onOpen: (plan: Plan) => void; canApprove: boolean; onApprove: (plan: Plan) => void }) {
  ...
  return <div className="module-page"><ModuleHead eyebrow="Administración · sólo administrador" title="Usuarios, roles y configuración de REVENUE" description="Administra personas, responsables, cuentas y permisos. Monitoreo es transversal; Construcción sólo corresponde al responsable del Plan y a Administración." />
    ...
    {!plan && existingPlans.length > 0 && <section className="paper-panel plan-picker"><b>Tus Planes</b><p>Continúa un Plan que ya empezaste; REVENUE te lleva exactamente donde lo dejaste.</p><div className="plan-picker-list">{existingPlans.map((item) => {
      ...
      return <article key={item.id}><div><b>{item.accountName ?? item.accountId}</b><small>{item.companyName ?? ""} · {item.year}</small></div><div><span>{statusLabels[version?.status ?? "DRAFT"] ?? version?.status ?? "Borrador"}</span>{canApprove && version?.status === "SUBMITTED" && <button type="button" className="clay-primary" onClick={() => onApprove(item)}>Aprobar comercialmente</button>}<button type="button" className="paper-button" onClick={() => onOpen(item)}>Abrir</button></div></article>;
    })}</div></section>}
