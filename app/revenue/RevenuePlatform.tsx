...
export default function RevenuePlatform({ identity, initialModule = "monitoreo" }: { identity: RevenueIdentity; initialModule?: RevenueModule }) {
  ...
  return (
    <Shell active={active} identity={effectiveIdentity} onNavigate={navigate} showSteps={Boolean(selected) && !creating} onStepNavigate={goToStep}>
      ...
      active === "revision" ? selected ? <ReviewModule plan={selected} baseline={state.review} baselineResult={state.baseline} growth={state.growth} result={state.result} profitability={state.profitability} priorYear={state.priorYear} priorYearGrossSales={state.priorYearGrossSales} synthetic={syntheticPlan} busy={busy} onSubmit={submit} /> : <NoPlan onCreate={startCreate} /> :
