...
export function GrowthPlanModule({ family, plan, contributions, growth, source, synthetic, canBuild, canContribute, canIntegrate, waitingFor, busy, onUpload, onBuild, onContribute, onDecide }: {
  ...
  return <div className="module-page">
    ...
    {activities.length ? <>
      {canBuild && <div className="inline-actions"><button type="button" className="paper-button" disabled={Boolean(busy)} onClick={onBuild}>{busy ? "Recalculando…" : "Recalcular Marketing y Trade"}</button></div>}
...
export function ProfitabilityModule({ profitability, files, onUpload, ready, busy, onBuild, readOnly = false }: { profitability: ProfitabilityResult | null; files: ReceivedFile[]; onUpload: (requirementId: string, file?: File) => void; ready: boolean; busy: string; onBuild: () => void; readOnly?: boolean }) {
  ...
  const [measure, setMeasure] = useState<"money" | "units">("money");
  return <div className="module-page">
    ...
    {profitability ? <>
      ...
      <section className="plain-note"><b>Comparadores declarados</b><p>El P&amp;L muestra año anterior, Real/Actual y Plan actual. Las dos variaciones quedan separadas como Diferencial: Plan contra año anterior y Real/Actual contra Plan.</p><label>Vista<select value={measure} onChange={(event) => setMeasure(event.target.value as "money" | "units")}><option value="money">Dinero</option><option value="units">Unidades</option></select></label></section>
      {measure === "units" && <section className="pnl-statement"><div className="pnl-header"><b>Volumen</b><b>Año anterior</b><b>Real / Actual</b><b>Plan</b><b>Plan − año anterior</b><b>Real − Plan</b></div><div className="pnl-row"><b>Unidades</b><span>{profitability.unitComparison.prior?.toLocaleString("es-MX") ?? "—"}</span><span>{profitability.unitComparison.actual?.toLocaleString("es-MX") ?? "—"}</span><span>{profitability.unitComparison.plan.toLocaleString("es-MX")}</span><strong>{profitability.unitComparison.planVsPrior?.toLocaleString("es-MX") ?? "—"}</strong><strong>{profitability.unitComparison.actualVsPlan?.toLocaleString("es-MX") ?? "—"}</strong></div></section>}
      {measure === "money" && (profitability.priorYearAnnual || profitability.actualAnnual) ? <section className="pnl-statement"><div className="pnl-header"><b>Renglón financiero</b><b>{profitability.priorYear ?? "Año anterior"}</b><b>Real / Actual</b><b>Plan actual</b><b>Plan − año anterior</b><b>Real − Plan</b></div>{([["Gross sales","grossSales"],["− Condiciones comerciales","deductions"],["= Net sales","netSales"],["− Costo","cogs"],["= Margen bruto","grossMargin"],["− Inversión","investment"],["= Contribución","contribution"]] as const).map(([label,key]) => <div className="pnl-row" key={key}><b>{label}</b><span>{profitability.priorYearAnnual ? formatMoney(profitability.priorYearAnnual[key], profitability.currency) : "—"}</span><span>{profitability.actualAnnual ? formatMoney(profitability.actualAnnual[key], profitability.currency) : "—"}</span><span>{formatMoney(profitability.planAnnual[key], profitability.currency)}</span><strong className={(profitability.priorYearVariance?.[key] ?? 0) < 0 ? "negative" : "positive"}>{profitability.priorYearVariance ? formatMoney(profitability.priorYearVariance[key], profitability.currency) : "—"}</strong><strong className={(profitability.actualVariance?.[key] ?? 0) < 0 ? "negative" : "positive"}>{profitability.actualVariance ? formatMoney(profitability.actualVariance[key], profitability.currency) : "—"}</strong></div>)}</section> : measure === "money" ? <section className="plain-note warning"><b>Comparadores pendientes</b><p>El Plan se puede calcular, pero aún no hay fuentes históricas o Real/Actual comparables con cobertura financiera completa.</p></section> : null}
