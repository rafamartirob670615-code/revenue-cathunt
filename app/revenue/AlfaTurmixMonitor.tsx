...
type Body = { dataset: { label: string; category: string }; cutoff: { asOfDate: string; label: string }; options: Record<string, string[]>; matrix: MatrixBlock[]; totals: { actualValue: number; acceptedPlanValue: number; coverage: number | null; vsLastYearValue: number; vsLastYearPercent: number | null } };

const filterKeys: FilterKey[] = ["territory", "account", "accountGroup", "channel", "subchannel", "family", "product", "period"];
const labels: Record<FilterKey, string> = { territory: "Ruta", account: "Cuenta", accountGroup: "Agrupación", channel: "Canal", subchannel: "Subcanal", family: "Familia", product: "Producto", period: "Periodo" };
const filterHints: Partial<Record<FilterKey, string>> = { territory: "Cuentas que atiende un vendedor" };
...
export default function AlfaTurmixMonitor() {
  ...
  if (loading && !body) return <main className="revenue-content"><div className="platform-loading"><span /><b>Abriendo estado del negocio…</b></div></main>;
